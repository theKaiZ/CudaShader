...
__device__ float map(vec3 p){ 
  float plane = sinfr*1.5 + 3.5-p.z;
  float bulb1 = abs(mandelbulb(p));
  float bulb2 = abs(mandelbulb2(p));
  float circle = abs(dist(p,vec3(cosfr*0.5,sinfr*0.5,sinfr*0.125)+vec3(0,0,0.15+0.25*sinfr))-1.5+0.5*cosfr)-0.25+0.15*sinfr;
...
__global__ void Mandel_calc(unsigned char* image_buffer){
  ...
  float y0 = - (float) (row -window.x/2)/(window.x/2)*1.4;
  float x0 = (float) (col -window.y/2)/(window.y/2)*1.4;
  float r,g,b;  
  ...
  vec3 ambient = vec3(0.1,0.1,0.65);
  c3po = c3po + ambient + vec3(1,1,1);

  r = c3po.x*100-p.x;
  g = c3po.y*100-p.y*p.z;
  b = c3po.z*100-p.z;
